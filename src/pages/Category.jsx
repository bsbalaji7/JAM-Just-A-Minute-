...
import { products, upcomingEvents } from '../data/products';
...
const Category = () => {
  ...
  const categoryTitle = searchParams.get('title') || categoryType || 'All Items';
  const isUpcomingEventsOnly = categoryTitle === 'Upcoming Events';

  const mappedUpcomingEventProducts = upcomingEvents.map((event, index) => ({
    id: `ue${index + 1}`,
    ...
    rating: 'NEW',
    deliveryTime: `${event.date} | ${event.time}`,
    image: event.image,
    description: `${event.location}. ${event.description}`
  }));

  const filteredProducts = isUpcomingEventsOnly
    ? mappedUpcomingEventProducts
    ...
    <div className="py-10 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-4xl font-black text-gray-900 tracking-tight">{categoryTitle}</h1>
            <p className="text-gray-500 font-medium">Found {filteredProducts.length} items in this category</p>
          </div>
        </div>

        {!isUpcomingEventsOnly && (
